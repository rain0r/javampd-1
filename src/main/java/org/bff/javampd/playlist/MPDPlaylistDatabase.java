 ...
 * obtain an instance of the class you must use the {@link org.bff.javampd.database.MusicDatabase#getPlaylistDatabase()}
...
public class MPDPlaylistDatabase implements PlaylistDatabase {
  ...
  public Collection<MPDSong> listPlaylistSongs(String playlistName) {
    ...
    List<MPDSong> songList = songConverter.getSongFileNameList(response)
        .stream()
        .map(song -> new ArrayList<>(songDatabase.searchFileName(song)))
